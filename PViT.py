...
class PViT(nn.Module):
    def __init__(self, *, args, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.prior_token_position = args.prior_token_position
    ...
    def forward(self, img, prior):
        ...
        prior_token = self.prior_projection(prior_normalized)
        prior_token = prior_token.unsqueeze(1).expand(b, 1, -1)
        ...
        return (self.mlp_head(x), attention_maps) if self.draw else self.mlp_head(x)


    # def forward(self, img, prior):
    #     x = self.to_patch_embedding(img)
    #     # x = self.prior_embedding(x, prior) 

    #     b, n, _ = x.shape

    #      # Project the prior and expand to the batch size
    #     prior_normalized = F.softmax(prior, dim=-1)
    #     prior_token = self.prior_projection(prior_normalized).unsqueeze(1).expand(b, -1, -1)
    #     # Apply the scaling factor to the prior token
    #     # prior_token *= torch.sigmoid(self.scale_factor)  # Using sigmoid to keep the scale factor between 0 and 1
    #     prior_token *= self.scale_factor
    #     cls_tokens = repeat(self.cls_token, '1 1 d -> b 1 d', b = b)
    #     if self.prior_token_position == 'start':
    #         x = torch.cat((prior_token, cls_tokens, x), dim=1)
    #     elif self.prior_token_position == 'second':
    #         x = torch.cat((cls_tokens, prior_token, x), dim=1)
    #     elif self.prior_token_position == 'end':
    #         x = torch.cat((cls_tokens, x, prior_token), dim=1)
    #     else:
    ...
    #     if self.draw:
    #         return self.mlp_head(x), attention_maps
    #     else:
