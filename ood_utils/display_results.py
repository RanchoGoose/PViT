...
def plot_histogram(id_scores, ood_scores, threshold=None, save_path="histogram.png"):
    # Plot histograms
    plt.hist(id_scores, bins=50, alpha=0.5, label='In-Distribution (ID)', color='blue')
    plt.hist(ood_scores, bins=50, alpha=0.5, label='Out-of-Distribution (OOD)', color='red')

    # If a threshold is provided, draw a vertical line at that value
    if threshold is not None:
        plt.axvline(x=threshold, color='green', linestyle='--', label=f'Threshold: {threshold:.2f}')

    # Set titles, labels, and legends
    plt.title("Score Distributions of ID vs. OOD Data")
    plt.xlabel("Score")
...
def optimize_threshold(y_true, y_scores, recall_level=recall_level_default):
    ...
    for threshold in thresholds:
        ...
        if np.isclose(recall, recall_level, atol=1e-2) and fpr < min_fpr:
...
def fpr_and_fdr_at_recall(y_true, y_score, recall_level=recall_level_default, pos_label=None):
    ...
    # # Optimize threshold
    # opt_threshold = optimize_threshold(y_true, y_score, recall_level=recall_level)
...
def get_measures(_pos, _neg, recall_level=recall_level_default):
    ...
    # fpr, optimized_threshold = fpr_and_fdr_at_recall(labels, examples, recall_level)
    
    # plot_histogram(_pos, _neg, optimized_threshold, "./figs/histogram.png")
